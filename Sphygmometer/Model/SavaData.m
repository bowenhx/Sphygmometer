...
- (NSInteger)printDataInteger:(NSString*)key{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
//    LOG(@"defaultw*********     = %@",[defaults dictionaryRepresentation]);
    NSMutableDictionary *dict = [self dataForUser:[self currentUid]];
    if (dict) {
...
+(void) writeArrToFile:(NSArray *)arr FileName:(NSString *)file
{
    ...
    NSString *filename=[path stringByAppendingPathComponent:file];
     ...
     BOOL isFinish = [arr writeToFile:filename  atomically:YES];
    NSLog(@"数组写入文件状态为 = %d",isFinish);
}
...
//计算用户年龄
+ (NSInteger)getUserYearCountNumber:(NSString *)birthday
{
    //获取当前年份
    NSDate *date = [NSDate date];
    NSDateFormatter *formatter = [self userDateFormatter];
    NSString *times = [NSString stringWithFormat:@"%@",[formatter stringFromDate:date]];
    NSString *timesNum = [times substringToIndex:4];
    NSInteger today = [timesNum integerValue];
    
    //出生年月
    NSString *birStr = [birthday substringToIndex:4];
    NSInteger ages = [birStr integerValue];
    
    //计算用户年龄
    NSInteger year = today - ages;
    return year;
}
//存储用户ID
